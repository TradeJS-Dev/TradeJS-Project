...
test("builds a profiled cross-strategy report from tiny sharded exports", async (t) => {
  ...
  const sourceRoot = await fsp.mkdtemp(
    path.join(os.tmpdir(), "ai-cross-source-"),
  );
  ...
  await fsp.mkdir(path.dirname(pocketModulePath), { recursive: true });
  await fsp.writeFile(
    pocketModulePath,
    "module.exports = { searchAiPockets: () => ({ positivePockets: [], negativePockets: [] }) };\n",
    "utf8",
  );
